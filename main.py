...
def processQueries(query):
    ...
    top10 = [{'title': r['Title'], 'url': r['Url'], 'desp':r['Description']} for r in top10]

    if len(top10) < 10:
	return top10, parsed_result 

    for skip in range(0, 1001, 50):
    	bingUrl = 'https://api.datamarket.azure.com/Bing/Search/Web?Query=%27' + quoted_query + '%27&$top=50&$format=json' + '&$skip=' + str(skip)
